...
public class Nav1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (SceneManager.GetActiveScene().name != "WelcomeScen_webAR" && SceneManager.GetActiveScene().name != "LanguageSelect_webAR" && SceneManager.GetActiveScene().name != "WelcomeSceneHindi_webAR")
            {
                if(Utils.langMode== Utils.LanguageMode.English)
                    GoToScene("WelcomeScen_webAR");
                else
                    GoToScene("WelcomeSceneHindi_webAR");

            }
                
            else
                GoToScene("LanguageSelect_webAR");
        }
    }
